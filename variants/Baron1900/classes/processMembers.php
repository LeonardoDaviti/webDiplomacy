...
class Baron1900Variant_processMembers extends processMembers {
    ...
    function checkForWinner()
    {
        ...
        else
        {
            ...
            foreach($this->ByStatus['Playing'] as $Member)
            {
				// LOCAL (issue 009 wave 3): $Game->targetSCs and $Game->maxTurns are vDiplomacy-only
				// Game properties (a per-game solo target and turn limit); webDiplomacy has neither,
				// and reading them is a fatal under this install's error handler. Guarded so the
				// variant falls back to its own $supplyCenterTarget, which is what it means anyway.
				if ( isset($this->Game->targetSCs) && $this->Game->targetSCs > 0 )
				{
		...
		if ( isset($this->Game->maxTurns) && ($this->Game->turn == ($this->Game->maxTurns - 1)) && ($this->Game->maxTurns > 0) )
			return $this->check_for_Winner_that_works_with_same_SC_count();
