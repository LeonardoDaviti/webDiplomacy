...
class HavenVariant_panelGameBoard extends panelGameBoard
{
	...
	function mapHTML() {
		...
		if (( file_exists($staticPNG) ) or ( file_exists($staticMAP) ))
			/* LOCAL DEVIATION (issue 009 wave 5): STATICSRV is a vDiplomacy-only constant
			   naming its static-content host; webDiplomacy has no such constant and reading
			   it is a fatal in PHP 8. This install serves the static map from its own
			   webroot, so the prefix is simply empty. */
			$smallmapLink = $staticPNG.'?nocache='.$this->processTime;
